...
## Echify Implementation Strategy

> [!NOTE]
> Echify does **NOT** need to use `disable_stripe_user_authentication` since **Stripe manages the liability** for connected accounts. This means Stripe's default authentication will be used, which is the recommended approach for most platforms.

### Current State

Echify currently uses **Account Links** for Stripe Connect integration, which:
- ❌ Redirects users away from the app to Stripe-hosted pages
- ⚠️ Limited to onboarding and account update flows only
- ⚠️ Does not provide embedded dashboard experience

### Required Components

Echify only needs two primary components:
1. **Account Onboarding** - For new connected account setup
2. **Account Management (Dashboard)** - For viewing/editing account details

### Recommended Approach

Given time constraints, we recommend a **hybrid approach**:

```mermaid
flowchart TB
    subgraph Backend["Backend Changes"]
        AS["Switch to Account Sessions API"]
    end
    
    subgraph iOS["iOS Implementation"]
        iOSOnboard["Account Onboarding<br/>(Native SDK)"]
        iOSAccount["Account Management<br/>(Native SDK)"]
    end
    
    subgraph Android["Android Implementation"]
        AndroidOnboard["Account Onboarding<br/>(Native SDK)"]
        AndroidAccount["Account Management<br/>(WebView - Web Embedding)"]
    end
...
| Platform | Account Onboarding | Account Management |
|:---------|:-------------------|:-------------------|
| **iOS** | ✅ Native SDK ([flutter_stripe_connect](https://pub.dev/packages/flutter_stripe_connect)) | ✅ Native SDK |
| **Android** | ✅ Native SDK ([flutter_stripe_connect](https://pub.dev/packages/flutter_stripe_connect)) | 🌐 WebView (Stripe Web Embedding via `connect.echify.com`) |

### Why This Approach?

1. **Account Onboarding** - Both iOS and Android have native support via `flutter_stripe_connect`
2. **Account Management**:
   - ✅ **iOS** has native SDK support
   - ❌ **Android** does NOT have native Account Management in the SDK
   - 🌐 **Solution**: Use Stripe Web Embedding with WebView for Android

### Implementation Steps

1. **Backend**: Switch from Account Links to Account Sessions API
   ```typescript
   // Before (Account Links)
   const accountLink = await stripe.accountLinks.create({...});
   
   // After (Account Sessions)
   const accountSession = await stripe.accountSessions.create({
...
2. **Flutter App (iOS & Android Onboarding)**:
   - Use `flutter_stripe_connect` package for Account Onboarding
   - Works on both platforms with native SDK

3. **Flutter App (Account Management)**:
   - **iOS**: Use `flutter_stripe_connect` native Account Management
   - **Android**: Open WebView to `connect.echify.com/account?publishableKey=xxx&clientSecret=xxx`

### Benefits

- ✅ Minimal implementation effort (only 2 components needed)
- ✅ Native experience on iOS for both features
- ✅ Native onboarding on Android
- ✅ Web fallback for Android Account Management
- ✅ Stripe manages authentication and liability (no extra platform responsibility)
- ✅ No need to implement all 25 Stripe components
